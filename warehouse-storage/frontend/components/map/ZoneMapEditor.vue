      ...
      >
        ...
        >
          <line
            ...
            :x1="wallLinePoints(wall).x1"
            :y1="wallLinePoints(wall).y1"
            :x2="wallLinePoints(wall).x2"
            :y2="wallLinePoints(wall).y2"
            stroke="#cbd5e1"
            stroke-width="8"
...
// Walls are stored as a rect + rotation (see useWallGeometry.ts); this view
// only draws their centerline for background orientation context.
function wallLinePoints(wall: WallInput) {
  return wallToEndpoints(wall)
}
