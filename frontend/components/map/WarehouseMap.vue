<template>
  <div ref="scrollContainer" class="scrollbar-slim overflow-x-auto pb-1.5">
    <div
      ...
      :style="{
        gridTemplateColumns: `56px repeat(${layout.shelf_numbers.length}, 52px)`,
      }"
    >
      <!-- Column header row -->
      ...
      <div
        ...
        :key="`col-${n}`"
        class="flex h-8 items-center justify-center text-[0.75rem] font-bold text-muted"
      >
        {{ n }}
      </div>
      ...
      <template v-for="level in layout.levels" :key="`row-${level}`">
        <div
          class="flex h-8 w-14 items-center justify-center text-[0.75rem] font-bold text-muted"
        >
          {{ level }}
        </div>
        <button
    ...
    <div class="mt-3.5 flex flex-wrap gap-[18px]">
      <span class="flex items-center gap-1.5 text-[0.8rem] text-muted"
        ><i
          class="inline-block h-2.5 w-2.5 rounded-[3px] border border-edge bg-surface"
        ></i>
        Empty</span
      >
      <span class="flex items-center gap-1.5 text-[0.8rem] text-muted"
