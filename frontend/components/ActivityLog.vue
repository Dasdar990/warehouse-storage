<template>
  <section class="card">
    <div class="mb-3.5 flex items-center justify-between">
      <h2 class="text-[1.05rem]">📋 Registro Movimenti</h2>
      <button
        type="button"
    ...
    <p v-if="loading && movements.length === 0" class="py-4 text-muted">Caricamento registro…</p>
    <p v-else-if="movements.length === 0" class="py-4 text-muted">Nessun movimento registrato.</p>
      ...
      <table class="w-full min-w-[640px] border-collapse text-[0.85rem]">
        <thead>
          <tr class="border-b border-edge text-left text-[0.72rem] uppercase tracking-wide text-muted">
            <th class="pb-2 pr-3 font-semibold">Ora</th>
            <th class="pb-2 pr-3 font-semibold">Operatore</th>
            <th class="pb-2 pr-3 font-semibold">Azione</th>
            <th class="pb-2 pr-3 font-semibold text-right">Qtà</th>
            <th class="pb-2 pr-3 font-semibold">Item / P/N</th>
            <th class="pb-2 font-semibold">Origine</th>
          </tr>
        </thead>
        <transition-group tag="tbody" name="log-row">
          ...
          >
            ...
            <td class="whitespace-nowrap py-2.5 pr-3">
              ...
              >
                {{ m.action === 'deposit' ? '🟢 Depositato' : '🔴 Prelevato' }}
              </span>
            </td>
            ...
            <td class="py-2.5 pr-3">
              ...
              <div class="text-[0.75rem] text-muted">P/N {{ m.pn }} · Scaffale {{ m.shelf_position }}</div>
            </td>
            <td class="whitespace-nowrap py-2.5">
              <span
                ...
                title="Operazione confermata da scansione barcode"
              ...
              <span
                ...
                title="Operazione inserita manualmente da interfaccia"
              >⚠️ Manual Entry</span>
            </td>
          </tr>
        </transition-group>
