...
def list_recent_movements(
    ...
    *,
    ...
    if date_from is not None:
        stmt = stmt.where(Movement.timestamp >= datetime.combine(date_from, time.min, tzinfo=timezone.utc))
    if date_to is not None:
        stmt = stmt.where(Movement.timestamp <= datetime.combine(date_to, time.max, tzinfo=timezone.utc))
